...
namespace Library
{
	...
	void ProxyModel::Initialize()
	{
		...
		DX::ThrowIfFailed(mGame->GetD3DDevice()->CreateVertexShader(&compiledVertexShader[0], compiledVertexShader.size(), nullptr, mVertexShader.GetAddressOf()), "ID3D11Device::CreatedVertexShader() failed.");
		...
		// Load a model
		std::unique_ptr<Model> model = std::make_unique<Model>(mModelFileName, true);
	...
	void ProxyModel::Update(const DX::StepTimer& gameTime)
	{
