...
namespace Library
{
	...
	void BlendStates::Initialize(ID3D11Device* direct3DDevice)
	{
		...
		DX::ThrowIfFailed(direct3DDevice->CreateBlendState(&blendStateDesc, AlphaBlending.GetAddressOf()), "ID3D11Device::CreateBlendState() failed.");
		...
		DX::ThrowIfFailed(direct3DDevice->CreateBlendState(&blendStateDesc, MultiplicativeBlending.GetAddressOf()), "ID3D11Device::CreateBlendState() failed.");
	}
